...
object Entrance extends App {
  ...
  override def main(args: Array[String]) {
    val spark = SparkSession
      .builder()
      .appName("CSE512-HotspotAnalysis-BRAINIACS") // YOU NEED TO CHANGE YOUR GROUP NAME
      .config("spark.some.config.option", "some-value")
      //.master("local[*]")
      .getOrCreate()

    //val args = "dummy "+"test/output hotcellanalysis src/resources/yellow_trip_sample_100000.csv"split(" ")
